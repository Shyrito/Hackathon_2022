...
class Applicationdb{
    ...
    public function getAllApplicationWithUserIdFilter($id,$gender,$grade)
    {
        if($gender!=null && $grade!= null){
            ...
            try {
                $bdd = DBLink::connect2db(MYDB, $message);
                $result = $bdd->query("SELECT * FROM " . self::TABLE_NAME . " JOIN CANDIDATE ON CANDIDATE.id_candidate = APPLICATION.fk_candidate where fk_job_offer = '$id' AND gender = '$gender' AND grade = '$grade' ", PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "APPLICATION\APPLICATION");
            } catch (Exception $e) {
                $message .= $e->getMessage() . '<br>';
            }
            DBLink::disconnect($bdd);
            ...
            try {
                $bdd = DBLink::connect2db(MYDB, $message);
                $result = $bdd->query("SELECT * FROM " . self::TABLE_NAME . " JOIN CANDIDATE ON CANDIDATE.id_candidate = APPLICATION.fk_candidate where fk_job_offer = '$id' AND gender = '$gender'", PDO::FETCH_CLASS | PDO::FETCH_PROPS_LATE, "APPLICATION\APPLICATION");
            } catch (Exception $e) {
                $message .= $e->getMessage() . '<br>';
            }
            DBLink::disconnect($bdd);
